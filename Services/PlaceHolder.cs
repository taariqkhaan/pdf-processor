    ...
    public void AnnotatePdf(string pdfPath, string outputFolder)
{
    ...
    while (hasMoreRecords)
    {
        string query = @$"
            ...
            FROM pdf_table
            WHERE Tag IN ('facility_name', 'facility_id', 'dwg_title1','dwg_title2', 'dwg_scale', 
        ...
        hasMoreRecords = reader.HasRows;
        if (!hasMoreRecords) break;

        Dictionary<int, XGraphics> pageGraphicsCache = new();
        ...
        while (reader.Read())
        {
            ...
            if (!pageGraphicsCache.TryGetValue(pageIndex, out XGraphics gfx))
            {
                gfx = XGraphics.FromPdfPage(page);
                pageGraphicsCache[pageIndex] = gfx;
            }
            ...
            XColor outlineColor = missingValue ? XColor.FromArgb(0, 255, 0, 0) : XColor.FromArgb(0, 255, 230, 0);
            XColor fillColor = missingValue ? XColor.FromArgb(50, 255, 0, 0) : XColor.FromArgb(80, 255, 230, 0);

            XPen outlinePen = new XPen(outlineColor, 1);
            XSolidBrush brush = new XSolidBrush(fillColor);

            double outlineOffset = 1.5;
            double adjustedX1 = x1 - outlineOffset;
            double adjustedRectY = adjustedY2 - outlineOffset;
            double adjustedWidth = rectWidth + 3;
            double adjustedHeight = rectHeight + 3;
        ...
        // Dispose XGraphics instances after batch processing
        foreach (var gfx in pageGraphicsCache.Values)
        {
            gfx.Dispose();
        }
